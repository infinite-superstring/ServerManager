...
def get_node_warning_count() -> int:
    ...
    count = Node.objects.filter(
        Q(node_event__level__in=["Warning", "Error"]) & Q(node_event__end_time=None)).distinct().count()
    return count
...
def get_import_node_list_excel_object():
    ...
    cols = [
        ...
        ExcelColumn("节点标签列表（英文逗号分隔）", 'str', validate=ColumnValidate(null=False, max=256)),
        ExcelColumn("节点备注", 'str', validate=ColumnValidate(null=False, max=256)),
        ExcelColumn("集群", 'select',
                    validate=ColumnValidate(null=False, select=[group.name for group in Node_Group.objects.all()])),
        ExcelColumn("启用节点登录限制", 'bool', validate=ColumnValidate(null=False)),
        ExcelColumn("节点登录限制方法", 'select', validate=ColumnValidate(null=False, select=["限制IP", "限制网段"])),
        ExcelColumn("节点登录限制值", 'str', validate=ColumnValidate(null=False)),
    ]
