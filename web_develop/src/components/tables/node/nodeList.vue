          ...
          <v-card>
            <v-card-title class="machine-name">
              ...
              <div class="machine-action">
                <v-btn size="x-small" @click="$emit('action:reset_token', item.id)">重置Token</v-btn>
                <v-btn size="x-small" color="red" @click="$emit('action:del_node', item.id)">删除节点</v-btn>
              </div>
            </v-card-title>
            <v-card-text>
              <v-row>
...
export default {
  ...
  emits: ['action:del_node', 'action:reset_token'],
  data() {
    return {
      text: "测试文本"
    }
